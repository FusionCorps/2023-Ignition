    ...
    public void execute() {
        if (!reachedPrep) {
            ...
            if (mArm.getWristPos() < MID_WRIST_POS) {
                mArm.passSetpoints(HIGH_BASE_POS_ALT, HIGH_WRIST_POS_ALT);
                System.out.println("Trying to end");
            } else {
                mArm.passSetpoints(HIGH_BASE_POS_ALT_PREP, HIGH_WRIST_POS_ALT);
            }
        }

        if (!reachedPrep && abs(mArm.getBasePos() - HIGH_BASE_POS_ALT_PREP) <= BASE_ERROR_THRESHOLD) {
            reachedPrep = true;
            mArm.configBaseAccel(BASE_MAX_A/5);
        }
    }

    @Override
    public boolean isFinished() {
        return abs(mArm.getWristPos() - HIGH_WRIST_POS_ALT) < 1000 &&
                abs(mArm.getBasePos() - HIGH_BASE_POS_ALT) < 1000;
    }
